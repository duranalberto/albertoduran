...
export class BuildLogger {
  ...
  private logInit(): void {
    const disableWorker = process.env.MERMAID_DISABLE_WORKER === "true";
    const hasWorkerUrl = !!process.env.MERMAID_RENDERER_URL;
    const primaryService = disableWorker
      ? "mermaid.ink (Worker disabled via env)"
      : hasWorkerUrl
        ? "Cloudflare Worker (sequential queue)"
        : "mermaid.ink (Worker URL not set)";
    ...
    if (hasWorkerUrl && !disableWorker && process.env.MERMAID_RENDERER_URL) {
      console.log(
        `${dim(t)} ${magenta("[mermaid]")} Worker URL: ${cyan(process.env.MERMAID_RENDERER_URL)}`,
      );
    }

    console.log(
      `${dim(t)} ${magenta("[mermaid]")} Debug logging: ${process.env.DEBUG_MERMAID === "1" ? cyan("ENABLED") : dim("disabled")} (set DEBUG_MERMAID=1 for verbose output)\n`,
    );
  }

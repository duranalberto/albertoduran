...
`src/components/ui/mdx/EChart.astro` is the author-facing boundary. A publication passes an ECharts `option`, dimensions, text metadata, and optional render or hydration preferences. The component normalizes those values before anything reaches the renderer.
...
```mermaid
flowchart TD
    Source[MDX EChart component] --> Normalize[Normalize render and hydration]
...
Playwright covers the browser contract through `/fixtures/charts/` when `ALBERTODURAN_TEST_MODE=true`. It verifies static SVG without JavaScript, external SVG artifacts, opt-in enhancement, and media-query hydration. The dummy gallery under `src/thejournal/echarts_dummy_gallery.mdx` gives humans a full article-shaped chart bench.

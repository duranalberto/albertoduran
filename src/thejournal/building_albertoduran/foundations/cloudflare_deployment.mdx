...
The build warning belongs in the same review. The deterministic audit completed successfully but reported minified JavaScript chunks above 500 kB. Static hosting does not make a large client chunk free, so deployment verification should include which routes request those bundles and whether optional ECharts hydration contributes to them.

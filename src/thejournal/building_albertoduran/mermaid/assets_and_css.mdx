...
description: "How Mermaid SVG assets, page CSS hoisting, diagram expansion, and renderer version guardrails protect production output."
...
The Mermaid system does more than replace code fences with inline SVG. It also emits standalone theme-specific assets, uses those assets as cacheable diagram pages, and hoists per-diagram CSS into page-level CSS. That gives readers fast inline diagrams and gives the build stable asset URLs for reuse.
...
This system is fast because it caches aggressively. It reuses memory cache, disk cache, remote production assets, and immutable standalone SVG files. That speed comes from treating diagram output as content-addressed build output.

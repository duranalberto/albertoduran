...
description: "Concurrent MDX files register diagrams with one build-scoped coordinator that handles cache lookup, batch rendering, and asset emission."
...
The inline SVG remains in article HTML. The standalone assets power Open Diagram links and production cache reuse. That split gives readers immediate diagram markup and gives the build a durable reuse target for unchanged diagrams.
...
## Keep registration build-scoped

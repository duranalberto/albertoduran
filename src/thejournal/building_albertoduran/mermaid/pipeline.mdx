...
description: "How the Mermaid diagram pipeline coordinates concurrent MDX files, cache layers, batch rendering, and static SVG asset emission."
...
The Mermaid pipeline exists because every unnecessary render has a cost. It can cost build time, Browser Rendering quota, public fallback patience, and cache stability. `DiagramPipeline` is the build-scoped coordinator that receives Mermaid fences from MDX, checks caches, batches render requests, and records the assets that need to be emitted. It keeps unchanged diagrams stable across builds.

---

## Pipeline state

`createPipeline()` returns a build-scoped `DiagramPipeline` instance. The integration creates it at `astro:build:start`, passes its `registerDiagram()` function to the remark plugin, and clears the reference after `astro:build:done`.
...
```mermaid
flowchart TD
    Register[📌 Register diagram] --> Memory{⚡ Memory cache}
...
Remote cache lookup is a static-site trick. If production already has the exact SVG asset for a diagram, the next build can fetch it, strip standalone-only CSS, sanitize it, and reuse it as the inline node. The generated site becomes part of the build cache.

---

## Batch orchestration

MDX files can process concurrently, so diagrams may register from many files at nearly the same time. The pipeline collects those registrations before it flushes a render batch.

`FLUSH_DEBOUNCE_MS = 800` gives concurrent remark pipelines time to register diagrams. `CHUNK_SIZE = 40` keeps Worker payloads within the expected size. `INTER_CHUNK_DELAY_MS = 22000` spaces Worker chunk requests so the rendering service is not treated like an infinite local function.
...
The inline SVG remains in article HTML. The standalone assets power Open Diagram links and production cache reuse. That split gives readers fast inline diagrams and gives the build a durable cache target for unchanged diagrams.
...
The fixture renderer still produces SVG maps for the requested themes. The rest of the pipeline still registers diagrams, merges theme output, emits assets, and hoists CSS. That keeps the build shape close to production while replacing the network-dependent rendering step.
...
## Registration from MDX

The pipeline starts when MDX is processed. Mermaid code fences are not rendered directly by the browser. The remark layer registers their source with the `DiagramPipeline`, and the article receives a placeholder that can later become rendered SVG.

That registration step is important because the build needs to know all diagrams before it can batch renderer requests and emit assets. A diagram is both article content and a future static asset. The pipeline gives it an identity before the renderer sees it.
...
After rendering, the transform layer receives theme-specific SVG strings and builds one merged article node. That node contains the inline diagram the reader sees. Asset emission writes light and dark standalone versions for Open links. Page CSS hoisting later reduces repeated style blocks in the built HTML.

The pipeline therefore connects several outputs from one source fence. Inline article SVG, standalone light SVG, standalone dark SVG, page CSS, cache entries, and runtime data attributes all trace back to the same diagram source.
...
The Mermaid pipeline turns a code fence into a set of static artifacts. It registers diagrams during MDX processing, reuses cached SVG where possible, renders through provider contracts, transforms output into article-safe nodes, emits standalone assets, and supports deterministic fixture builds.
...
The pipeline acts as build memory. As MDX files are processed, diagrams appear at different moments. The pipeline collects them, gives them identities, tracks cache keys, stores render output, and later emits assets after Astro has generated routes.

That memory is necessary because Mermaid output is used in more than one build phase. The article needs inline SVG during rendering. The asset hook needs to write standalone files later. The build summary needs to know what happened. Page CSS hoisting needs final HTML at the end.
...
Astro hooks place each step at the moment when enough information exists. At build start, the integration can prepare caches. During MDX processing, it can register diagrams. At build generated, it can emit assets into the output directory. At build done, it can scan completed HTML for CSS hoisting.

This hook order keeps the work honest. The integration does not try to emit assets before it knows what diagrams exist. It does not hoist CSS before pages exist. It does not wait until runtime to do work that the build can finish.

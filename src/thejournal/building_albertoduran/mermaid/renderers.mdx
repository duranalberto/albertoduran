...
description: "How the Mermaid renderer chain prefers a self-owned batch Worker and falls back to the slower public mermaid.ink service."
...
The renderer chain has two jobs. Prefer the fast service that I control, then use a public renderer when that path is unavailable. Both renderers produce SVG strings, but they have different payload contracts, rate behavior, and transform requirements. The site hides those differences behind one `fetchDiagrams()` call.
...
Because it is self-owned, the Worker can also encode the service constraints the site cares about: authentication, request size limits, theme isolation, and browser reset behavior. Those concerns belong near the renderer, not in the Astro article route.

...
description: "How the site turns DaisyUI-aligned palettes into Mermaid base theme variables with contrast helpers and memoized output."
...
The renderer service receives Mermaid config, not site CSS. The transform layer receives SVG strings, not palette objects. Runtime receives inline SVG and standalone asset links, not Mermaid token maps.
...
Contrast helpers exist because diagram text and shapes can combine in many ways. A node fill can be light or dark. Text may need to sit on primary, secondary, base, or mixed colors. The theming layer chooses readable foregrounds before the renderer produces SVG.
...
This makes accessibility and visual fit part of the diagram build, not a post-render overlay.
...
The integration renders both versions and emits standalone assets for both. Runtime can then point Open links at the correct asset based on `html[data-theme]`. Inline diagrams and standalone diagrams share the same theme source.
...
Mermaid theming is translation. Site palettes become Mermaid variables. Contrast helpers choose readable pairings. Mix helpers create diagram-specific surfaces that still belong to the local color family. Memoization keeps repeated theme generation cheap.
...
Runtime theme switching still matters for Open links and wrappers, but the expensive theme application happens during rendering. That keeps diagrams fast for readers and consistent for the build.

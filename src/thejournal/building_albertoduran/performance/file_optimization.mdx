...
flowchart TD
    ...
    class JsCss,Css,Pages,DiagramCss,ChartSvg,Icons,Dist output
```
...
The audit build also reported minified chunks above 500 kB. That warning is the most concrete JavaScript signal currently available, and it remains unresolved. The next step is to map those chunks to routes, set an explicit budget, and measure the effect before describing the output as light.

...
Inter comes from Fontsource as `--font-inter`. Montserrat comes from Fontsource as `--font-montserrat`. Fira Code loads from a local WOFF2 file as `--font-fira-code`. The Astro font configuration defines weights, subsets, display behavior, and local source metadata.

...
description: "The active Zod schema accepts less metadata than the MDX files carry, exposing fields that are unsupported, unused, or only partly wired."
...
The active schema keeps the frontmatter contract compact. It supports metadata for cards, headers, routing context, and sorting without asking every post to repeat values the build can infer. The source files currently carry one field that the contract does not accept, which is exactly the kind of drift a schema article should expose.
...
| `description`   | String with a default.          | Cards, the layout manifest, and Open Graph description. The standard description meta tag currently receives the page title instead. |
| `pubDate`       | Required date after coercion.   | Article header and sorting contexts.                                                                                                 |
| `updatePubDate` | Optional date after coercion.   | Mapped to `updatedDate` in the manifest, but not rendered by `PublicationHeader.astro`.                                              |
...
### Fields that currently go nowhere

All 41 files in this vault previously declared `author`, but `src/content.config.ts` has no `author` field and the article UI does not render a byline. The rewrite removes that unsupported frontmatter instead of advertising a contract the application does not have. Adding a byline later requires a schema field, manifest type, UI consumer, and test.

`updatePubDate` has a different problem. The schema accepts it and the manifest maps it to `updatedDate`, yet `PublicationHeader.astro` displays only the publication date. The data is partly wired rather than unsupported. A contributor should either finish that route to the UI or remove dates that imply readers can see an update marker.

The description path exposes a third mismatch. `BaseLayout.astro` passes `siteManifest.description` to the Open Graph description, while the standard `<meta name="description">` receives `siteManifest.pageTitle`. Search previews may therefore miss the article summary even though the frontmatter is valid. The case study records that defect rather than describing the intended behavior as finished.

---

## The accepted update date
...
The validation rule captures the data relationship, but it does not finish the reader experience. A valid update date can still disappear before presentation. Schema validity proves that an input is shaped correctly; it does not prove that a component consumes it.

...
description: "The publication system behaves like a compiler whose policy stages turn MDX candidates into routes, navigation, and complete article pages."
...
Charts now follow the same no-import instinct. A data visualization still needs values and chart metadata, but authors write those as strict JSON inside an `echart` fence. The integration renders static SVG at build time before any browser enhancement is considered.

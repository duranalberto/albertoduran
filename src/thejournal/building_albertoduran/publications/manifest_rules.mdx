...
After entries become contexts, `buildJournalManifest()` groups them by vault. It creates a `rootVaults` bucket for each first-level folder, places the vault index first, and then asks `buildNestedStructure()` to turn the bucket into a tree.

Every vault root must have an index. Every nested section must also have an index. If a folder contains entries but no index file, the builder throws a descriptive error.

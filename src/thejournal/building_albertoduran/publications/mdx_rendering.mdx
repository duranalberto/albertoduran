...
description: "How Astro renders compiled MDX entries and how component overrides shape code blocks, headings, tables, and Mermaid shells."
...
An MDX article starts as a file that a person can read in a text editor. MDX is Markdown with the option to use components, which makes it useful for technical writing that still needs article-specific UI. The rendered page is much richer. Headings copy links, tables scroll with keyboard access, code blocks have line numbers and theme-aware tokens, and Mermaid diagrams ship as SVG instead of raw source. The trick is to keep the authoring surface plain while letting the route substitute site components at render time.
...
| `h3` | `HeadingAnchorH3` | Preserve H3 semantics while reusing heading anchor behavior. |
...
The H3 case needs a small wrapper. `HeadingAnchor` defaults to rendering an `h2`, so mapping both `h2` and `h3` to the same component would collapse subheadings into the wrong semantic level. `HeadingAnchorH3.astro` forwards props and passes `as="h3"`.

```astro
<HeadingAnchor as="h3" {...props}><slot /></HeadingAnchor>
```

That wrapper exists for accessibility as much as convenience. Screen readers and outline tools should see the same heading hierarchy the author wrote in MDX.
...
- Avoid importing local UI components unless the article needs a special interactive or visual element.

...
description: "How Playwright validates browser-only behavior such as overlays, responsive layouts, view transitions, themes, and Mermaid popovers."
...
flowchart TD
    ...
    class Smoke,Responsive,Overlay,Theme,Article,Mermaid browser
```
...
## Mermaid popovers and no-JS behavior
...
No-JS behavior is also part of the browser layer. The site is static-first, so important content should remain available without runtime enhancement. Browser coverage can load pages with JavaScript disabled and confirm that core content remains visible.
...
No-JS coverage is important because the site is built static-first. Article text, headings, links, code blocks, tables, and inline diagrams should not require client-side rendering. Runtime modules can add convenience, but the content should already be present.

That promise shapes the whole build. Mermaid renders during the build. The manifest prepares route context. The Atlas widget has a build-time fallback display. Overlay content exists in the HTML. Browser tests with JavaScript disabled make that design promise observable.

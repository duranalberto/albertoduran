...
flowchart LR
    ...
    class Manifest,Mermaid,Minifier,Atlas test
    class Routes,Diagrams,Html,Data output
```

The common thread is that these checks protect project rules, not only library behavior. Astro, Mermaid, and html-minifier-terser all do useful work, but the site has local expectations about how their output should fit the publishing system.

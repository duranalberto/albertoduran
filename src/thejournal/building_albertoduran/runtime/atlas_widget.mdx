...
description: "How the Atlas widget fetches schedule data during the build, falls back deterministically in tests, and localizes time in the browser."
...
The data shape is deliberately small. It contains standing, record, points, home team, away team, raw date, stadium, and city. Those fields are exactly what the visual component needs. The loader does the translation from external API shape into local display shape before the component renders.
...
`atlas-schedule` localizes the schedule time in the browser.
...
The Atlas widget shows how live data can belong in a static site. The external API is contacted during the build. The response is normalized into a small content entry. The component renders static HTML with optimized images and a Mexico City time fallback. The browser enhances only the timezone label.

That is the same architecture used throughout the site. Durable data and layout decisions happen before deployment. The browser adds the part that depends on the reader. The result feels alive without asking every page view to rebuild the widget.

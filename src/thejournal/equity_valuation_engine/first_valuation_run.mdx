...
<Steps
  aria-label="First valuation workflow"
  currentStep={4}
  items={[
...
<Callout variant="information" title="About the examples">
  <p>Commands use the repository's documented ORCL example. Displayed sample output is fictional and frozen, so it demonstrates interpretation without claiming current market facts.</p>
</Callout>
...
<MockupWindow aria-label="Fictional suitability output" bodyClass="p-5 font-mono text-sm">
  <strong slot="header">Suitability excerpt</strong>
  <pre className="overflow-x-auto whitespace-pre-wrap">{`DCF          run      normalized inputs usable
...
P/E          excluded historical price and EPS dates need alignment`}</pre>
  <span slot="caption">Skipped and editorially excluded models remain visible.</span>
</MockupWindow>

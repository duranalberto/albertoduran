...
```daisyui
{
  ...
  "content": [
    {
      "type": "paragraph",
      "text": "The registry preserves many important numeric and derived failures, not every absence. Label and enum misses can return None without entries. Optional historical series may also remain unrecorded."
    }
  ]
}
```
...
```daisyui
{
  "component": "callout",
  "variant": "caution",
  "title": "Use matching currencies",
  "content": [
    {
      "type": "paragraph",
      "text": "The engine records financial and trading currency labels, but the valuation path does not call the existing currency service. Statement values and share prices can therefore be combined without conversion."
    }
  ]
}
```

...
class OnThisPage {
  ...
  private refreshHeaderOffset(): void {
    ...
    // Use offsetHeight (layout dimension, transform-independent) rather than
    // getBoundingClientRect().height which would be wrong if the header carries
    // a CSS transform.
  ...
  private syncFromScroll(): void {
    ...
    if (activeIdx !== -1 && tops[activeIdx] < 0) {
    ...
    this.setActive(activeIdx !== -1 ? this.entries[activeIdx].link : null);
  }
  ...
  private bindEvents(): void {
    ...
    // Chrome iOS: visualViewport.resize fires immediately when the browser
    // toolbar hides/shows (changing visual viewport height). window.resize
    // fires asynchronously after layout reflows and can miss the timing window,
    // causing heading position queries to run against stale dimensions.
    // The rAF ensures getBoundingClientRect calls happen after the new
    // viewport dimensions have been applied to the layout.
  ...
  destroy(): void {
    ...
    // Manual removal — visualViewport has no AbortSignal support on older iOS
